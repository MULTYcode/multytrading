...
namespace App\Http\Controllers\Tamo;

use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use App\tamouser;

class TamoUserCtrl extends Controller
{
    ...
    protected function login(Request $request){
        ...
        $login = tamouser::where('email', $email)->first();
        if (!$login) {
            $res['error'] = true;
            ...
            if ($hasher->check($password, $login->password)) {
                $api_token = sha1(time());
                $create_token = tamouser::where('id', $login->id)->update(['api_token' => $api_token]);
                if ($create_token) {
                    $res['error'] = false;
